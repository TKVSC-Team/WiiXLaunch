...
## Rainbow quad rendered solid black: `NVNvertexAttribState`/`NVNvertexStreamState` were 2x their real size

With the pipeline fully working, the rainbow quad (position + per-vertex color, `shaders/rainbow.vert`/`.frag`, a plain passthrough) rendered as a solid black quad instead of red/green/blue/yellow. `DrawBnshQuadDirect` had already been stripped down to bind zero custom blend/color/depth/polygon/channelMask state (comment in `nvn_overlay.hpp`: "we previously got yellow (wrong state offsets), now solid black... strip all state object binds, inherit the game's last-bound state for everything") - so the black output wasn't coming from pipeline state at all.

Root cause: this file's opaque `NVNvertexAttribState`/`NVNvertexStreamState` struct definitions were never verified against the real SDK header the way `NVNprogram`/`NVNsync` explicitly were (see "BREAKTHROUGH" section above) - they'd been guessed at `0x8`/`0x10` bytes. The real `nvn.h` (`NvnHeader/nvn/nvn.h:274-281`, a local SDK copy found at `C:\Users\dylan\repos\NVN\NvnHeader\nvn\nvn.h`) says `NVNvertexAttribState` is **4 bytes** and `NVNvertexStreamState` is **8 bytes** - both exactly half what this file declared. Confirmed independently via Ghidra against real game code: `sead::PrimitiveDrawMgrNvn::prepareFromBinaryImpl` (`0x7100b01310`) lays out three `NVNvertexAttribState` objects exactly 4 bytes apart (`param_1+0x218/+0x21c/+0x220`) and its one `NVNvertexStreamState` spans exactly 8 bytes (`+0x228` to `+0x230`, where the next distinct object starts).

`nvnCommandBufferBindVertexAttribState(cmdBuf, numAttribs, attribs)` (`nvn.h:11951`) walks `attribs` as a tightly-packed array using the REAL struct size. With `g_RainbowVertexAttribStates` declared as `NVNvertexAttribState[2]` using the old (doubled) 8-byte struct, calling `bindVertexAttribState(cmdBuf, 2, g_RainbowVertexAttribStates)` meant the driver read "element 1" (location 1, `a_Color`) from byte offset 4 - squarely inside element 0's (`a_Position`, location 0) unused padding, never touching the real color-attribute data actually written to `g_RainbowVertexAttribStates[1]`. The driver treats that slot as effectively disabled/zeroed, so `a_Color` always fetched `(0,0,0,0)`, `v_Color` was always black, and the fragment shader dutifully output solid black - position (element 0, offset 0) was unaffected, which is why the quad still drew in the right place and shape, just with no color.

Fixed both structs to their real sizes (`nvn_overlay.hpp`). Also fixed the five pipeline-state structs (`NVNblendState`/`NVNcolorState`/`NVNchannelMaskState`/`NVNpolygonState`/`NVNdepthStencilState`, previously `uint64_t[8]` = 64 bytes each vs. real 4-8 bytes per `nvn.h:241-271`) - same defect class, harmless today only because every current call site binds a single instance rather than an array, but would bite the moment any of them are ever arrayed.

Rebuilt clean via `build_switch.bat` (PowerShell, not `cmd.exe` - see the build-pipeline-bug section above), redeployed to the Ryujinx mods folder. Confirmed working - the quad renders as an actual rainbow (red/green/blue/yellow triangles) in Ryujinx.

## General shader packer: `scripts/pack_shader.py`

With the struct-size bug fixed and the rainbow quad confirmed rendering correctly, generalized the hand-built `rainbow_sead_bin.hpp` process into a real tool so any GLSL vertex/pixel shader pair can go through the same pipeline, not just the one hand-packed shader.

Reverse-engineered the exact packing recipe by diffing raw `ShaderConverter.exe --separable` output (`shaders/test_rainbow.bnsh`) against the known-working `rainbow_sead_bin.hpp` byte-for-byte in Python:

- The control blob baked into `rainbow_sead_bin.hpp` (byte-identical for both vertex and fragment sections) is a **verbatim, unmodified copy** of the first GLSLC control-magic-tagged (`0x98761234`) block in the raw compiler output, taken up to (but not including) the second occurrence of that same magic. Confirmed reproducible: compiling `rainbow.vert`/`rainbow.frag` fresh produced a byte-identical control blob to the one already embedded, just at a different file offset (the surrounding sections' sizes shift slightly between compiles; the tagged block's content itself doesn't).
- Each stage's compiled SASS code is tagged with a DATA magic (`0x12345678`). Found the exact byte sequence already embedded in `rainbow_sead_bin.hpp`'s code sections inside the raw `.bnsh` at fixed offsets, confirming: first data-magic hit = vertex code, second = fragment code, and ShaderConverter lays each stage's code out in a fixed-size aligned slot (empirically 0x200 bytes here, consistent across two independent compiles of the same source) - reusing that slot size for the last stage's bound (rather than scanning to EOF) matters, since there's real non-zero trailing data after the code (debug/variation bookkeeping) that a naive "trim to EOF" scan would wrongly include.
- At load time only the version + device-ISA words in the control blob are patched (`NvnOverlay::LoadSeadBinaryProgram`, née `GetBnshProgram`) - everything else is used exactly as compiled.

`scripts/pack_shader.py <vertex.glsl> <fragment.glsl> <OutputName>` runs `ShaderConverter.exe -s Glsl -c Binary -a Nvn --separable`, extracts both regions per the above, and emits `include/<name>_sead_bin.hpp` in the same array-literal style as `rainbow_sead_bin.hpp`. Self-tested by round-tripping `rainbow.vert`/`.frag` through it before trusting it with new content.

## Demo: animated plasma shader, zero new NVN binding code

Wrote `shaders/plasma.vert`/`.frag` (a classic multi-sine-wave "plasma" effect, screen-space via `gl_FragCoord`) and packed it with the new tool. To animate it without adding any new binding plumbing (no UBO, no texture), repurposed the existing per-vertex color attribute's `.a` channel as a "time" value, written from the CPU into the vertex buffer every frame exactly the way vertex color already was (`DrawPlasmaQuadDirect` in `nvn_overlay.hpp`, mirroring `DrawBnshQuadDirect`). Factored the shared load sequence out into `LoadSeadBinaryProgram` so both the rainbow and plasma programs can be loaded into their own independent static GPU resources (`GetBnshProgram`/`GetPlasmaProgram`). `HookedCommandBufferEndRecording` now draws the plasma quad instead of the rainbow one.

## Plasma shader failed to bind (`ProgramSetShaders result=0`): fixed code-slot-size assumption was wrong

First real test: nothing rendered, no crash. Log showed `Rainbow ProgramSetShaders result=1` (still fine) but `Plasma ProgramSetShaders result=0` - the driver rejected the plasma program at bind time. Since `GetPlasmaProgram`/`DrawPlasmaQuadDirect` both correctly no-op on a null program, this failed safely (nothing drawn, nothing corrupted) rather than crashing - but meant the demo was invisible.

Root cause: `pack_shader.py`'s original `extract_code_regions` assumed every stage's compiled code lives in a **fixed-size slot**, sized from the one data point available at the time (the gap between rainbow's vertex and fragment data-magic markers, 0x200 bytes) and reused for every stage including the last. Recompiled plasma's raw `.bnsh` standalone and diffed it byte-for-byte against a fresh rainbow recompile in Python: total container size was identical (12552 bytes both - ShaderConverter pads `--separable` 2-stage output to a fixed size, independent of shader complexity), but the two files differed in a 576-byte cluster starting at file offset `0x2280` and running to `0x24bf` - well past the `0x2400` boundary my fixed 512-byte fragment slot cut the extraction off at. Inspecting those extra bytes directly showed dense SASS-looking instruction bytes followed by what are almost certainly plasma.frag's own float literals (`0.012`, `6.28318`, `3.0`, `3.3`, `2.1`, `1.7`, `4.0`, etc.) baked into an inline constant pool appended right after the code, with no marker of its own - a real, necessary part of the program that my fixed-size cap was silently truncating.

First fix attempt: a `region_length` boundary detector that scans forward from each stage's data-magic marker for the first 64-byte run of zero bytes. Validated against rainbow (reproduced the known-good 320/256 lengths exactly) - but still failed to bind (`ProgramSetShaders result=0` again) after redeploying. Went back in with the same byte-level diff technique: plasma's real fragment data turned out to have a **second**, separate 16-byte constant-pool chunk sitting exactly 64 zero bytes after the main 704-byte code block (decoded: `0.012`, `~0.04`, `~0.036`, `2.1` - all real literals from `plasma.frag`), followed by a genuine ~2800-byte zero gap before the container's own unrelated trailing data (confirmed again by finding a literal filename string, this time `plasma_raw`, inside it). The 64-byte zero-run threshold landed exactly on that internal gap and stopped one chunk too early.

A single zero-run-length threshold turned out to have no value that works for both boundaries: long enough to clear internal gaps (64 bytes, confirmed on plasma) risks running out of room before an early stage's real, tight boundary (rainbow's vertex stage only has ~192 zero bytes before the fragment stage starts - a 256-byte threshold there just falls through to "no run found, use it all," silently reintroducing over-inclusion). Replaced the heuristic with an exact rule instead: whenever a stage has a **real** next boundary (the next stage's own data-magic marker - stages are packed back-to-back, so this is unambiguous, not a guess), bound the scan there and take the last non-zero byte directly, no threshold needed. Only the true last stage lacks such a marker; there, a fixed 2048-byte cap (`SAFE_CAP`) stands with wide, checked margin between every observed real content length (audience: hundreds of bytes) and every observed real start of the container's trailing section (~2800-3500 bytes out).

Re-validated against both known points before trusting it again: rainbow reproduces exactly 320/256 (unchanged), plasma now correctly captures all 784 real bytes (704-byte code + the second 16-byte constant chunk, previously truncated twice over). Repacked `plasma_sead_bin.hpp`, rebuilt clean, redeployed. Still `ProgramSetShaders result=0`.

## Real root cause: code sections need 256-byte alignment, not just correct content

Two straight fixes to the code-extraction length and it still failed - time to question the premise instead of the length. Added `GetRainbowSelftestProgram`, a diagnostic-only third program that loads `pack_shader.py`'s own repack of the *exact same* `rainbow.vert`/`rainbow.frag` source the hand-built `rainbow_sead_bin.hpp` was built from - never drawn, just loaded alongside the other two so one relaunch's log shows all three bind attempts side by side. Result: `RainbowSelftest ProgramSetShaders result=0` too, right next to `Rainbow result=1` for the byte-content-identical source. Conclusive: the bug was never about plasma's complexity or code length at all - `pack_shader.py`'s overall file *layout* was broken from the very first version, and every earlier fix had only ever verified extracted byte *content*, never that a fully repacked file actually binds.

Compared the two files' 16-byte headers directly: the hand-built file's code sections sit at `0x1200`/`0x1400` - both exact multiples of `0x100` (256). `pack_shader.py` had been placing code immediately after the two control blocks with zero padding, landing at `0x1110`/`0x1250` - neither aligned. Real GPU shader code commonly requires this kind of alignment for instruction fetch; the hand-built file (originally constructed by hand, unknowingly following this constraint) happened to already respect it, while the generalized tool didn't.

Fixed `pack_sead_binary` to round each code section's offset up to the next 256-byte boundary, padding with zeros in between (`CODE_ALIGNMENT`/`align_up` in `pack_shader.py`). Re-verified before redeploying: the repacked rainbow selftest now has **byte-identical header offsets** (`0x10`/`0x890`/`0x1200`/`0x1400`) and **byte-identical code content** to the hand-built file. Rebuilt, redeployed. **Still `ProgramSetShaders result=0`, for both RainbowSelftest and Plasma.**

## The actual real root cause: vertex and fragment need DISTINCT control blobs, not one shared copy

Every fix so far had chased code-region content, then code-region alignment - both real bugs, both necessary, neither sufficient. With header offsets and code content now byte-identical to the working file and it still failing, the only thing left to suspect was the control blob's own 248-byte tail that had been visible-but-dismissed since the very first byte-diff investigation, several sections up. Time to actually look at it instead of assuming it didn't matter.

Diffed the hand-built file's real vertex control section against its real fragment control section directly (`orig[0x10:0x890]` vs `orig[0x890:0x1110]`) - and they are **not** identical: 31 bytes differ, at relative offsets `0x6f8`-`0x734` (several small per-stage fields, including `0x714`/`0x734` going `0->1` - exactly the "stage bucket" field this session's much earlier Ghidra decompile of the real driver had already identified, independently) and a 24-byte block at `0x7d0`-`0x7e7` that looks like a per-stage content fingerprint (completely different, high-entropy bytes between the two stages). This directly contradicts an assumption baked into `pack_shader.py` since its very first version: that the two control-magic-tagged blocks in the raw compiler output are byte-identical duplicates, safe to collapse into one and reuse for both stages.
